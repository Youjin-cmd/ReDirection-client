...
- [🏔 Challenges](#🏔-challenges)
  ...
  - [2. 자바스크립트 환경에서 비디오와 이미지 다루기](#2-자바스크립트-환경에서-비디오와-이미지-다루기)
    ...
    - [2-3 분석시 추출하는 프레임을 최대한 가볍게](#2-3-모션-분석부터-결과-반환까지)
...
<img src="https://github.com/Youjin-cmd/ReDirection-client/assets/83858724/8d5d7bd3-0588-40fb-8d9a-1cc87211e92e" />
...
<img src="https://github.com/Youjin-cmd/ReDirection-client/assets/83858724/42d7d0c6-3281-4edc-8239-13a826e4653c" />
...
<img src="https://github.com/Youjin-cmd/ReDirection-client/assets/83858724/5410c93c-0ae5-4cb7-b2cc-56683d98dba3" />
...
<img src="https://github.com/Youjin-cmd/ReDirection-client/assets/83858724/6cb130db-cb69-408b-a156-c3c433f8b0ef" />
...
저는 따라서 초당 13매로 추출할 프레임의 개수를 절반가량으로 줄이고, 크기 또한 100 x 56으로 대폭 축소하였습니다.
...
먼저 프레임간의 비교를 위해서 영상을 이루는 프레임들을 추출합니다.
오직 분석용으로 쓰일 예정이므로 저퀄리티의 이미지들로 생성해줍니다.
...
안정적인 결과물을 얻을 수 있도록 배열내부의 좌표값을 최적화해줍니다. ([4번항목](#4-부드럽고-안정감있는-결과물-반환받기) 에서 더 자세히 다루었습니다.)

마지막으로 FFmpeg를 통해 실제 결과물의 재료가 될 고퀄리티의 프레임들을 추출해준 후

좌표배열을 기반으로 하여 Sharp를 이용해 프레임들을 크롭해줍니다.

크롭이 완료된 프레임들을 FFmpeg를 이용해 비디오로 합쳐줍니다.
이때에 미리 준비해놓았던 오디오 파일과 영상을 함께 먹싱해줍니다.
...
### 3-3. 좌표 배열을 부드러운 선형으로 만들어주기
  ...
  - 유저가 선택한 n을 분기점 삼아, 분기점들 사이를 선형으로 눌러주는 로직입니다.
  민감도의 강, 약을 조절하는 슬라이더 인터페이스를 통해 유저가 감도를 설정하면 어느정도 빠르기와 민감도로 카메라가 모션을 추적할지가 결정됩니다. n의 숫자가 낮으면 분기점 사이가 좁아져 추적이 정확하지만 다소 산만하며, n의 숫자가 높으면 분기점 사이가 멀어져 부드럽지만 덜 예민한 추적정도를 가지게 됩니다.

  슬라이더는 sensitive, medium, modest의 세가지 선택지를 제공하며, 사전에 실험을 통해 상식적인 결과물을 도출하는 숫자들로 할당하였습니다.
...
```
🔺 많은 시행착오 후 이제는 익숙해진 FFmpeg 커맨드 문법
...
- 또한 가운데 정렬의 개념도 FFmpeg는 알지 못하므로 FFmpeg 커맨드 문법을 이용하여 가운데계산식을 작성해야만 했습니다.

HTML관련 속성값들을 FFmpeg가 지원하는 형식에 걸맞게 바꿔주는 작업은 마치 서로 다른 언어로 쓰인 글의 번역작업같다고 느꼈습니다.
시행착오 끝에 클라이언트단에서 유저가 설정한 모습이 똑같이 영상에 삽입되어 반환되는 것을 두 눈으로 직접 확인했을 때는 큰 보람을 느낄 수 있었습니다.
