...
<p>
  <img src="https://github.com/Youjin-cmd/ReDirection-client/assets/83858724/aa1255ec-76da-4bf6-b1ed-29aec18ea633" />
</p>
...
- [🏔 Challenges](#🏔-Challenges)
  ...
  - [3. 분석시 추출하는 프레임을 최대한 가볍게](#3-분석시-추출하는-프레임을-최대한-가볍게)
  - [4. 크롭할 좌표를 최적화하여 부드러운 영상 만들기](#4-크롭할-좌표를-최적화하여-부드러운-영상-만들기)
  - [5. 글귀와 스티커를 어떻게 영상에 삽입할까?](#5-글귀와-스티커를-어떻게-영상에-삽입할까)
- [🗓 Schedule](#🗓-schedule)
- [📒프로젝트 소감](#📒-프로젝트-소감)
...
요즈음 트렌드인 인스타그램의 스토리나 틱톡에 가로영상을 올리려고 할 때면 그 중에 가장 맘에 드는 세로구간을 선택하면서 난감했던 적이 있을 것입니다. 춤을 추는 친구가 좌우로 크게 움직이거나, 결혼식에서 입장하는 부부가 우측 끝에서 좌측 끝으로 이동을 한다면, 전문 영상편집 프로그램이 제공하는 모션트랙킹을 이용하지 않는 이상 적당한 세로구간을 고르기가 어렵습니다.
...
| Jimp | Sharp |
| ------------ | ----------- |
| ? | C언어 기반으로 벤치마킹 성능이 매우 우수함 |
| ? | ? |

두 라이브러리의 장단점을 비교해 본 후 Sharp라이브러리가 수많은 비디오 프레임을 빠르게 처리해야하는 프로젝트에 딱 맞겠다고 판단하여 채택하였습니다. 또한 Sharp라이브러리에는 Canvas API와 마찬가지로 블렌딩을 위한 콤포지트 내장메서드도 존재하였기에 Canvas API를 기반으로 짠 모션분석의 과정을 그대로 실현시킬 수 있으리라 생각했습니다.

### 2-2. 모션 분석부터 결과 반환까지

FFmpeg와 Sharp를 이용한 모션분석의 과정을 그림으로 표현하면 아래와 같습니다.
상세히 하나하나 짚어보겠습니다.

먼저 프레임간의 비교를 위해서는 먼저 영상을 이루는 프레임들을 추출합니다.
오직 분석용으로 쓰일 예정이므로 저퀄리티의 이미지들로 생상해줍니다.

Sharp의 composite 메서드를 통해 두 이미지를 합성하여 변화한 구간만을 도출합니다.
옵션으로 "difference"를 주어 두 이미지 사이에 달라진 부분이 강조되도록 합니다.
움직임이 거의 없는 픽셀은 어두운 색깔을 띄고 움직임이 많은 픽셀은 밝은 색깔을 띕니다.

RGB는 픽셀이 가진 삼원색의 속성으로써 색상이 밝을 수록 RGB의 합계 스코어가 높습니다.
즉 위 이미지의 모든 픽셀의 RGB 스코어 분석을 하면 변화가 격렬했던 부분을 가려낼 수 있게 됩니다.

위와 같이 이미지를 스캔하듯 픽셀을 순회하며 스코어가 강한 세로줄의 위치를 배열에 담습니다.
배열에 담을지 여부를 가리는 기준점인 Threshold는 실험을 통해 얻은 적절한 숫자로 설정되어 있으며 픽셀이 단순 노이즈인지 모션에 해당하는지를 가려줍니다.

"모션분석 배열"의 중앙을 구한 후, 크롭의 기준점이 되는 좌측 최상단의 좌표값을 구하여 "좌표 배열"에 넣어줍니다.

안정적인 결과물을 얻을 수 있도록 배열내부의 좌표값을 최적화해줍니다. ([4번항목](#4-부드럽고-안정감있는-결과물-반환받기) 참조)

마지막으로 FFmpeg를 통해 실제 동영상을 이룰 프레임들을 추출해준 후

좌표배열을 기반으로 하여 Sharp를 이용해 프레임들을 크롭해줍니다.

크롭이 완료된 프레임들을 FFmpeg를 이용해 비디오로 합쳐줍니다.
이때에 미리 준비해놓았던 오디오 파일을 영상에 먹싱해줍니다.

결과물을 S3에 업로드하고 url을 이용하여 사용자가 결과물을 다운로드 받을 수 있게 합니다.

## 3. 분석시 추출하는 프레임을 최대한 가볍게
...
((선명한 사진들이 빽빽하게 겹쳐있는 모습))

예를 들어 25fps의 1280 x 720 영상이 있다고 할 때, 고작 15초 남짓되는 영상에서 추출되는 프레임은 384개입니다. 결과적으로 모션분석 시에 자바스크립트 로직이 순환해야하는 픽셀의 개수는 총 8847360000개로 어마어마한 분량이 됩니다.
그러나 피사체의 속도가 총알수준이 아닌 이상, 25분의 1초의 모션을 하나 하나 분석할 필요는 없으며, 이미지의 퀄리티가 낮더라도 큰 영역에 걸친 색상 차이를 분석하는데는 충분한 정보를 얻을 수 있습니다.

((화질이 낮은 사진들이 드문드문 겹쳐있는 모습))

저는 따라서 초당 13매로 추출할 프레임의 개수를 절반가량으로 줄이고, 크기 또한 100 x 56로 대폭 축소하였습니다.
이미지 형식 또한 PNG-24에서 PNG-8로 비트수가 적은 형식을 취하도록 했습니다.
...
## 4. 부드럽고 안정감있는 결과물 반환받기

((드문드문 있는 그래프 => 선이 이어져있는 모양의 그래프로))

백엔드로부터 전송받은 모션 좌표배열에 담긴 숫자는 단순 모션추적의 결과물이라, 좌표들이 서로간에 연결되어있지 않거나 끊어지는 문제가 있었습니다. 최적화 없이 프레임을 처리하는데 사용하게 되면, 사람이 관람하기에는 매우 불편하고 산만한 결과물이 반환되어 해결할 방법을 고민하게 되었습니다.
중구난방인 숫자의 분포를 부드럽게 이어주기 위해서, 세가지의 함수를 거쳐 배열 내부의 좌표들을 최적화해주었습니다.
...
2. **불규칙적으로 튀는 좌표를 눌러주기**

  ((이미지))

  - 예를 들어, [… , 10, 5, 10 , …], [… , 35, 10, 36 , …] 과 같이 두 숫자들의 사이에 급격하게 차이가 나는 수가 들어있을 경우, 가운데 숫자를 중간값으로 덮어씌우는 함수를 구축하였습니다. 이로 인해 급격히 카메라가 엉뚱한 곳으로 이동하는 부적절한 부분을 많이 줄일 수 있었습니다.

3. **좌표 배열을 부드러운 선형으로 만들어주기**

  ((sensitive의 결과물과 그래프))

  ((modest의 결과물과 그래프))

  - 유저가 선택한 n을 분기점 삼아, 분기점들 사이를 선형으로 채웠습니다.
  민감도의 강, 약을 조절하는 슬라이더를 도입해서 최적화 로직의 강도를 유저가 선택할 수 있도록 하였습니다. 상기의 최적화 로직 중 n번째 공배수마다 분기점을 삼고 선형으로 중간값들을 이어주는 함수에서, 이 n값은 유저가 슬라이더를 통해 직접 설정합니다. 따라서 어느정도 빠르기와 민감도로 카메라가 모션을 추적할지에 대해 유저의 주관이 반영되어 더욱 만족스러운 결과를 얻을 수 있게 했습니다.

  n값은 10, 15, 20의 세가지 선택지를 제공하며, 사전에 실험을 통해 상식적인 범위인지 여부를 측정하였습니다.
  결과적으로 배열 내부의 숫자가 부드러운 path를 이루게 되어 마치 카메라가 피사체를 쫓아 이동하는 듯한 안정적인 결과물을 반환할 수 있었습니다.

4. **유저의 주관 반영**

  ((이미지))

  프로젝트 초기 준비단계의 시뮬레이션을 통해 느낀 점은, 유저가 카메라를 가로로 드는 경우는 대부분, 피사체가 둘 이상이거나 좌우로 늘어선 것을 동시에 담고싶을 때란 것이었습니다. 그렇기에 둘 이상의 피사체가 영상 전반에 걸쳐 존재하는 케이스의 경우, 단순하게 모션이 일어난 구역의 중앙을 분석하는 현 로직으로는 유저가 만족스러워할만한 결과를 얻기에는 한계가 있었습니다.

  ((구역컨펌 gif))

  고민 끝에 사용자에게 희망하는 구역을 컨펌받는 단계를 도입하기로 하였습니다. 비디오 위에 영역지정 인터페이스를 작성하고 드래그 앤 드롭을 통해 자유자재로 조절이 가능하도록 했습니다. 사용자가 원하는 구역을 지정하고 제출할 경우, 영역의 내부에서만 모션분석 순회로직이 동작하도록 하였습니다.추가로, 유저가 지정한 영역이 결과 영상의 가로길이와 거의 동일한 경우는(3픽셀 차이 내) 모션 인식을 통한 카메라 이동을 희망하지 않는 것으로 보고, 픽스된 영상을 결과를 반환하도록 구성하였습니다.
...
## 5. 글귀와 스티커를 어떻게 영상에 삽입할까?

클라이언트단에서 글귀 & 스티커을 이용한 영상 꾸미기 인터페이스가 모두 완성되자, 정작 이들을 실제로 어떻게 영상에 삽입할 것인가라는 문제에 봉착했습니다. 반환되어 오는 결과물이 유저가 설정하고 제출한 모습과 오차 없이 정확하게 일치해야했기 때문이었습니다.
...
위를 참고하여, 클라이언트 측에서는 ffmpeg의 커맨드에 사용가능한 알맞은 데이터를 간추리고, 다음과 같이 서버에 전송하였습니다.
...
```
🔺 사용자로부터 제출받은 글귀와 스티커의에 관한 다양한 속성들.

또한, .ttf파일과 .png파일을 모두 S3에 업로드하고, edit 요청이 들어올 경우 ffmpeg의 작업을 들어가기 전에 필요한 파일들을 내려받도록 했습니다.

작업을 위한 재료들의 다운로드가 모두 완료되어, 글귀와 스티커를 영상에 삽입해야하는 경우 ffmpeg 커맨드 Argument는 아래와 같이 작성됩니다.
...
"-filter_complex", // 필터적용 커맨드. 이하는 파라미터.
...
// [1:v]... 두번째 미디어: 스티커
// [0:v]... 첫번째 미디어: 비디오
// ,drawbox= 글귀 뒤쪽에 위치할 박스를 작성.
// ,drawtext= 글귀를 작성.
"-y", // 덮어쓰기 허용
path.join(SAVING_DIR_EDITED_RESULT, "edited_video.mp4"), // 결과 비디오의 이름을 명명하고 저장할 경로를 지정
```
🔺 많은 시행착오 후 이제는 익숙해진 FFmpeg 커멘드 문법

html의 input태그의 속성에서 지원하는 “transparent”의 경우, ffmpeg가 다루는 속성명은 아니므로, 분기를 설정해주어 “transparent”가 아닐 경우에만 drawbox를 포함하여 커맨드를 작성하도록 했습니다.

HTML관련 속성값들을 ffmpeg가 지원하는 형식에 걸맞게 바꿔주는 작업은 마치 서로 다른 언어로 쓰인 글의 번역작업같아 무척 흥미로웠습니다. 시행착오 끝에 클라이언트단에서 유저가 설정한 모습이 똑같이 영상에 삽입되어 반환되는 것을 두 눈으로 직접 확인했을 때는 큰 보람을 느낄 수 있었습니다.
